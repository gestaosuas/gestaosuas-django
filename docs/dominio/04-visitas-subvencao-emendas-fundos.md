...
- **Emendas e Fundos**: mantém a estrutura original (igual ao que já existia antes desta sessão) — tem o campo **"Recurso"** (`emenda`) na seção 1 (Dados da Parceria) de ambos os relatórios; o Parecer Conclusivo tem só uma seção final "3. CONCLUSÃO" (campo `conclusao`); o Relatório Final não tem seção de conclusão separada.
  - **Exceção confirmada em 2026-09-23** (só no Relatório Final, só Emendas): o item "4." não tem mais um sub-item "Das atividades" — vira a) Dos objetivos, b) Das metas estabelecidas, c) Dos resultados, d) Da execução financeira (o campo `atividades` continua existindo no JSON/no model, só não é mais exibido nem editável pra essa diretoria — dado antigo já salvo fica órfão, sem uso). Subvenção não muda (continua a/b/c/d/e, com "c) Das atividades"). Abaixo do texto do item "5. Cumprimento do Objeto" e abaixo do texto de "Homologação da Comissão de Monitoramento e Avaliação", o PDF (`apps/directorates/pdf_documents.py`) passou a imprimir a data (`report_data.local_data`/`homologacao_local_data`, formato "Uberlândia, DD de mês de AAAA", alinhada à direita) — campos que já existiam no formulário web (`report_form.html`, com esse mesmo default gerado em `VisitReportView.get_context_data()`) mas nunca apareciam no PDF exportado, pra nenhuma diretoria; a impressão desses dois campos no PDF também ficou restrita a Emendas (Subvenção segue sem data no PDF, comportamento inalterado).
  - **Bug real corrigido no mesmo dia**: o default de `local_data`/`homologacao_local_data` só era aplicado quando `Visit.relatorio_final` estava **totalmente vazio** (`if not report_data:`, em `VisitReportView.get_context_data()`) — visitas cujo `relatorio_final` já tinha *qualquer* conteúdo (inclusive lixo de outro schema, ex. só chaves `item5_*` herdadas de um fluxo antigo de PSE, achado numa visita real de Emendas em produção/dev) caíam no ramo `else`, que só fazia backfill de `osc_name`. Resultado: a data nunca aparecia (nem no formulário, nem no PDF) pra qualquer visita nessa situação, mesmo já com o fix acima aplicado — reportado pelo usuário testando no dev ("não vi as datas"). Corrigido replicando o mesmo backfill (`if not report_data.get("local_data"): ...`) também no ramo `else`, igual já era feito pra `osc_name`.
...
**UI pra preencher esses 4 campos (CONFIRMADO 2026-08-24)**: ícone "Descrição do plano" no card de cada plano (`templates/directorates/monitoring/plan_list.html` e a aba "Plano de Trabalho" de `templates/monitoramento/_tab_content.html`), abre um modal ("Descrição do plano de trabalho": Objeto/Objetivos/Metas estabelecidas/Atividades) que salva via `WorkPlanObjectivesView` (`directorates:plan-objectives`, sem branch por diretoria). Existia só pra Emendas e Fundos desde 2026-07; estendido pra Subvenção a pedido explícito do usuário, e o ícone/modal antigo de Emendas (que o usuário reportou não abrir mais em produção, causa raiz não reproduzida com dados de teste) foi substituído pelo mesmo modal novo — hoje as duas diretorias compartilham o mesmo ícone/modal/rótulos.
...
| 2026-08-24 | B.4 atualizado: ícone "Descrição do plano" (Objeto/Objetivos/Metas estabelecidas/Atividades) estendido de Emendas e Fundos pra Subvenção também, unificado num modal só | Pedido inicial do usuário era só pra Subvenção (Emendas já tinha o recurso desde 2026-07); no meio da implementação o usuário reportou que o ícone existente de Emendas "não abre essas opções" em produção (causa raiz não reproduzida com dados de teste simples) e pediu pra estender o recurso novo pra Emendas também — o modal antigo (`plan_objectives_modal.html`) foi removido e substituído pelo novo em ambas as diretorias, unificando os rótulos (perdendo os prefixos legais "A)/B)/C)" que só Emendas usava) |
| 2026-08-27 | 3 correções em Monitoramento: (a) card de visita ganha indicador da diretoria primária de quem registrou, admin-only; (b) documento/PDF finalizado passa a mostrar a quantidade de "Lista de Espera" quando marcada; (c) `VisitRevertView` agora reseta `parecer_tecnico.status` junto com `visit.status` | 3 pedidos do usuário na mesma sessão. (a): "controle de quem é administrador", nova diretoria não precisa bater com a da visita (perfil pode estar vinculado a mais de uma). (b): bug real — campo já existia no formulário rascunho mas nunca tinha sido adicionado em `visit_document.html`/`pdf_documents.py`. (c): bug real — reverter só resetava o Instrumental, então re-finalizar mostrava o Relatório de Visita com o status antigo (finalizado) em vez de voltar como rascunho; corrigido com o mesmo padrão de `RevertReportView`, preservando o conteúdo já digitado |
| 2026-09-01 | Ajuste no indicador de diretoria (entrada 2026-08-27a): contas admin mostram "Admin" em vez de "Sem diretoria" | Pedido explícito do usuário: "temos alguns usuários administrador... configure para que quem for administrador lá no card das visitas apareça admin". `build_registered_by_directorate_map()` agora checa `is_superuser or role == "admin"` (mesma definição de `get_admin_user_ids()`) antes de cair no fallback da diretoria primária |
| 2026-09-01 | Selectbox admin-only nas páginas de Instrumental de Visita (Subvenção/Emendas e Fundos) pra filtrar os cards pela diretoria de quem registrou | Pedido explícito do usuário: "crie... um selectbox com todas as diretorias para filtrar as visitas por diretorias (somente admin)". Filtro 100% client-side (mesmo padrão do "Filtrar por OSC" já existente), reaproveita `visit.registered_by_directorate` (entrada 2026-08-27a/2026-09-01 acima) via `data-registered-directorate` nos cards — atributo só é emitido no HTML quando `is_admin_user`, pra não vazar a mesma informação que a pill já esconde de diretor/agente |
| 2026-09-23 | A.5-B atualizado: Relatório Final de Emendas e Fundos perde o sub-item "c) Das atividades" do item 4 (reletrado pra a/b/c/d) e o PDF ganha a data (Uberlândia, DD de mês de AAAA) abaixo do item 5 e abaixo da Homologação | Pedido explícito do usuário, só pra Emendas e Fundos ("não mexa no relatório final da Subvenção"). O campo `local_data`/`homologacao_local_data` já existia no formulário web com esse default (gerado em `VisitReportView`), mas nunca era impresso no PDF (`pdf_documents.py`) pra nenhuma diretoria — Subvenção continua sem essa data no PDF, comportamento não tocado |
| 2026-09-23 | Bug real corrigido: `local_data`/`homologacao_local_data` nunca eram preenchidos com o default pra uma visita cujo `relatorio_final` já tinha algum conteúdo (mesmo lixo de outro schema) | Reportado pelo usuário testando a mudança acima no dev ("não vi as datas") numa visita de Emendas cujo `relatorio_final` só tinha chaves `item5_*` (resíduo de outro fluxo) — `VisitReportView.get_context_data()` só fazia esse backfill quando o campo estava 100% vazio; corrigido replicando o mesmo padrão de backfill de `osc_name` já existente no ramo `else` |

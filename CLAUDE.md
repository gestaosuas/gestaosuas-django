...
**1b. Ajuste no mesmo dia seguinte (2026-09-01)**: pedido explícito do usuário — "temos alguns usuários administrador... configure para que quem for administrador lá no card das visitas apareça admin". Contas admin normalmente não têm `primary_directorate` nenhuma, então caíam sempre no fallback genérico "Sem diretoria", que não deixava claro que era um admin (por design — admin não é "de nenhuma diretoria específica", mas isso ficava ambíguo com "ninguém marcou uma diretoria pra essa pessoa"). `build_registered_by_directorate_map()` agora retorna `"Admin"` quando `profile.user.is_superuser or profile.role == "admin"` — mesma definição de admin já usada em `get_admin_user_ids()`, cobre tanto quem tem `role="admin"` quanto uma conta `is_superuser` sem esse role marcado (adicionado `select_related("user")` na query pra checar `is_superuser` sem N+1).

**1c. Selectbox pra filtrar pela mesma diretoria, admin-only (2026-09-01)**: pedido explícito do usuário, mesmo dia — "crie... um selectbox com todas as diretorias para filtrar as visitas por diretorias (somente admin)" nas páginas de Instrumental de Visita de Emendas e Fundos/Subvenção. Filtro 100% client-side (mesmo padrão já usado pelo campo "Filtrar por OSC", que também é JS puro sobre `.visit-card`), pra não precisar de round-trip ao servidor nem mexer em paginação: cada `.visit-card` ganhou `data-registered-directorate="{{ visit.registered_by_directorate|default:'Sem diretoria' }}"` (só quando `is_admin_user`, pra não vazar a informação no HTML de quem não deveria ver nem a pill — achado ao escrever o teste que confirma o atributo cru some do `response.content` pra não-admin, não só a pill visual) e um novo `<select id="registered-directorate-filter">` (`#inline-registered-directorate-filter` na aba inline) lista `all_directorates` (já existia no contexto das duas views) + duas opções extras fixas "Admin"/"Sem diretoria", casando com os 2 valores especiais que `build_registered_by_directorate_map()` pode retornar. JS: `visit_list.html` ganhou uma função `applyVisitCardFilters()` que combina os dois filtros (texto da OSC E diretoria) em vez de dois listeners independentes que se sobrescreviam; `home.html` (que já tinha `filterInlineVisitTable()` reexecutada toda vez que a aba "visits" é trocada via AJAX) recebeu o mesmo tratamento. Testado via Django test client (renderização do select com as opções certas, atributo `data-*` presente/ausente conforme o papel) e com navegador real: 3 visitas de Emendas e Fundos criadas por perfis de diretorias diferentes (Benefícios/CRAS/Admin), filtro por cada uma isolando exatamente o card certo, "Todas as diretorias" voltando a mostrar as 3 — nas duas superfícies (página avulsa e aba inline do dashboard), screenshots confirmando o layout do novo select ao lado do "Filtrar por OSC".
